...
lanip() {
  LANIP=$(ifconfig em0 | grep inet | awk '{print $2}')
  [[ -n $LANIP ]] && echo -n " $LANIP"
}

wlanip() {
  WLANIP=$(ifconfig iwm0 | grep inet | awk '{print $2}')
...
while true; do
  echo "%{l}  $(lanip)  |  $(wlanip)%{c}$(Clock)%{r}$(wifi)  |  $(CPU)  |  $(Memory)  |  $(Load)  |  $(Temp)  |  $(Battery)  "
  sleep 1
done
